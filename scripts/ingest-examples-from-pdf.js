/*
 * Extracts text from a PDF for a given page range and writes it to docs/examples.mdx.
 * Usage: node scripts/ingest-examples-from-pdf.js <pdfPath> <startPage> <endPage>
 */

const fs = require('fs');
const path = require('path');
// Use build for Node environment; legacy path can differ by version
...
async function extractTextFromPdf(pdfPath, startPage = 3, endPage = 34) {
  ...
  for (let p = startPage; p <= lastPage; p++) {
    ...
    // Build lines using hasEOL when available, otherwise join with spaces
    let currentLine = [];
...
async function main() {
  const [pdfPathArg, startArg, endArg] = process.argv.slice(2);
  ...
  const mdxPath = path.join(__dirname, '..', 'docs', 'examples.mdx');
